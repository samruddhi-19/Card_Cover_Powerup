...
import {
  ...
  gradientCss,
  coverFileName,
  resolveSelectionFromAttachment,
} from "../lib/covers.js";
import {
  renderCover,
  fetchImageAsDataUrl,
...
export default function CoverEditor({ t }) {
      ...
      try {
        ...
        try {
          ...
          if (card?.id) {
            ...
            // Restore selection: Prioritize saved meta selection (clean background),
            // or resolve from generated cover filename, or fall back to native Trello cover.
            let initialSelection = null;
            if (meta?.selection) {
              initialSelection = meta.selection;
              if (meta.selection.kind === "gradient") setTab("gradient");
              else if (meta.selection.kind === "solid") setTab("color");
            } else if (card.cover?.color) {
              const currentTrello = TRELLO_COLORS.find((c) => c.trello === card.cover?.color);
              if (currentTrello) {
                initialSelection = { kind: "solid", ...currentTrello };
              } else {
                const currentSolid = SOLID_COLORS.find(
                  (c) => c.hex?.toLowerCase() === card.cover.color?.toLowerCase() || c.id === card.cover.color
                );
                if (currentSolid) {
                  initialSelection = { kind: "solid", ...currentSolid };
                } else {
                  initialSelection = {
                    kind: "solid",
              ...
              // Check if the attachment was generated by Card Cover (e.g. card-cover-cream.png)
              const resolved = resolveSelectionFromAttachment(imageUrl);
              if (resolved) {
                initialSelection = resolved;
                if (resolved.kind === "gradient") setTab("gradient");
                else if (resolved.kind === "solid") setTab("color");
              } else if (imageUrl) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      await t.remove("card", "shared", "coverMeta").catch(() => {});
      t.closeModal();
    } catch (e) {
